var config = {
  ...
  scale: {
    ...
    width: Telegram.WebApp.viewportStableHeight * (9 / 16), // Assuming a 16:9 aspect ratio
...
function create() {
  ...
  this.cameras.main.setBackgroundColor(Telegram.WebApp.themeParams.bg_color);
  var self = this;
  this.socket = io();
...
// Example of handling the main button click in Telegram
Telegram.WebApp.MainButton.onClick = function() {
  Telegram.WebApp.sendData(JSON.stringify({ score: currentScore }));  // Sending score back to the bot
};

// Handling closure of the Mini App
Telegram.WebApp.onEvent('close', function() {
  console.log('Mini App is closing');
});

Telegram.WebApp.onEvent('viewportChanged', function(event) {
  if (event.isStateStable) {
    game.scale.resize(Telegram.WebApp.viewportStableHeight * (9 / 16), Telegram.WebApp.viewportStableHeight);
  }
});